    ...
    try:
        ...
        for row in res.get('interval', [])[1:]:
            ...
            try:
                ...
                elif 4.5 <= diff <= 5.5: await channel.send(f"⏳ **5 Minutes left** for **{row[0]}**!")
                elif 9.5 <= diff <= 10.5: await channel.send(f"📢 **10 Minutes left** for **{row[0]}**!")
            except: continue

        # 2. Cek Fix Boss (Data mulai baris ke-4/index 3)
        for row in res.get('fix', [])[3:]:
            if row[0] and hari in row[0].lower():
                try:
                    waktu_awal = row[1].split('/')[0].strip()
                    fix_dt = datetime.strptime(waktu_awal, "%H:%M").replace(year=now.year, month=now.month, day=now.day, tzinfo=pytz.timezone('Asia/Jakarta'))
...
async def fix(ctx):
    try:
        ...
        for row in res.get('fix', [])[3:]:
            if row[0] and hari in row[0].lower():
                waktu_text = row[1]
                waktu_mulai = waktu_text.split('/')[0].strip()
                embed.add_field(name=f"{row[2]}", value=f"🇮🇩 {waktu_text} WIB | 🇵🇭 {get_pht(waktu_mulai)} PHT", inline=False)
        await ctx.send(embed=embed)
