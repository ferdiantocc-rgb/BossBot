...
class BossView(discord.ui.View):
    ...
    async def confirm_death(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            await interaction.response.send_message("Gagal koneksi.", ephemeral=True)
    ...
    try:
        ...
        for row in res.get('fix', [])[4:]:
            if row[0] and hari in row[0].lower():
                try:
                    fix_dt = datetime.strptime(row[1].split('/')[0].strip(), "%H:%M").replace(year=now.year, month=now.month, day=now.day, tzinfo=pytz.timezone('Asia/Jakarta'))
